...
async function resolverFondoCesantias(identificacion) {
  const [rows] = await pool.execute(
    'SELECT `Cesantías` FROM `Maestro_Segmentación` WHERE `Identificación` = ? LIMIT 1',
    [identificacion]
  );
  return rows.length ? (rows[0]['Cesantías'] || '—') : '—';
}

function buildDatos(vin, ciudadDoc, fondoCesantias, responsable, firmaTrabajadorHtml) {
  return {
...
router.get('/:idVinculacion', async (req, res) => {
  try {
    ...
    const resultado = await validarTokenCRS(token, idVinculacion);
    if (!resultado.valido) {
    ...
    const fondoCesantias = await resolverFondoCesantias(vin['Identificación']);
    const plantilla    = await obtenerPlantilla('cesantias_retiro');

    // En la vista previa la firma del trabajador aparece como espacio vacío
    const datos   = buildDatos(vin, ciudadDoc, fondoCesantias, responsable, '');
    ...
    console.error('[firmarcesantias GET]', err);
...
router.post('/:idVinculacion', async (req, res) => {
  try {
    ...
    const datos     = buildDatos(vin, ciudadDoc, fondoCesantias, responsable, firmaTrabajadorHtml);
    ...
    const urlPdf    = await subirPDFCesantias(vin['Identificación'], idVinculacion, pdfBuffer);
    ...
    await pool.execute(
      `INSERT INTO Maestro_docTrabajador
       ...
       VALUES (?, 'PEND', ?, ?, ?, 'Retirado', ?, '60', 'CRS', ?, NULL, NULL, NULL, NULL, ?, ?)`,
    ...
    // Invalidar token
    await pool.execute(
      'UPDATE `Maestro_Vinculación` SET token_firma_crs = NULL, token_firma_crs_expira = NULL WHERE `Id Vinculación` = ?',
      [idVinculacion]
    );
