...
const FIRMA_HTML = path.join(__dirname, '../views/firmarcertificadoretiro/firma.html');
...
function buildDatos(vin, seg, responsable, firmaTrabajadorHtml) {
  return {
    ...
    fecha_retiro:          formatFechaCO(vin['Fecha de Retiro']),
    arl:                   (seg && seg.ARL)       || '—',
    eps:                   (seg && seg.EPS)       || '—',
    pension:               (seg && seg['Pensión']) || '—',
...
router.get('/:idVinculacion', async (req, res) => {
  try {
    ...
    const resultado = await validarTokenCT(token, idVinculacion);
    if (!resultado.valido) {
    ...
    const datos   = buildDatos(vin, seg, responsable, '');
    ...
    console.error('[firmarcertificadoretiro GET]', err);
