...
const { obtenerUrlFirmaReciente, subirPDFCesantias } = require('./storage');
...
async function verificarCRSExpirados() {
  try {
    const [rows] = await pool.execute(
      `SELECT v.\`Id Vinculación\`   AS idVinculacion,
              ...
              v.Usuario,
              s.\`Cesantías\`        AS fondoCesantias
       FROM \`Maestro_Vinculación\` v
       LEFT JOIN \`Maestro_Segmentación\` s ON s.\`Identificación\` = v.\`Identificación\`
       WHERE v.token_firma_crs IS NOT NULL
         AND v.token_firma_crs_expira IS NOT NULL
         AND v.token_firma_crs_expira < NOW()`
    );
    ...
    for (const r of rows) {
      try {
        ...
        const datos = {
          ...
          fondo_cesantias:        r.fondoCesantias || '—',
        ...
        const urlDoc    = await subirPDFCesantias(r.identificacion, r.idVinculacion, pdfBuffer);
        ...
        console.log(`[crsExpiryNotifier] CRS auto-generado (sin firma) para id=${r.idVinculacion}`);
      } catch (err) {
        console.error(`[crsExpiryNotifier] Error id=${r.idVinculacion}:`, err.message);
      }
    }
  } catch (err) {
    console.error('[crsExpiryNotifier] Error general:', err.message);
  }
}

module.exports = { verificarCRSExpirados };
