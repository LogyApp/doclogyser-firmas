...
async function runOptimization() {
  console.log('Starting DB index optimization v3...');
  const conn = await pool.getConnection();
  try {
    // 1. Optimize workers lookup from Maestro_Vinculación
    await addIndexSafe(conn, 'Maestro_Vinculación', 'idx_vinc_lookup_opt', '`Regional`, `Operación`, `Estado`, `Trabajador`(100)');

    // 2. Optimize client lookup from Maestro_Clientes_Credito
    await addIndexSafe(conn, 'Maestro_Clientes_Credito', 'idx_cc_cliente_facturar', '`Cliente a Facturar`(150)');

    console.log('Index optimization v3 finished successfully.');
  } catch (err) {
